...
namespace WinderProject
{
  class Bbox
  {
    ...
    */
    ...
    public static bool ActiveBreakOn { get { return activeBreakOn; }    //KSG Edit.  Added so the break state can be changed from Winder.StartLayer()
                                       set { activeBreakOn = value; } }
    private static bool inactiveBreakOn;  // false => off
    public static bool InactiveBreakOn { get { return inactiveBreakOn; }    //KSG Edit.  Added so the break state can be changed from Winder.StartLayer()
                                         set { inactiveBreakOn = value; } }
    private static bool recordOn;         // false => goto is on
    public static bool RecordOn { get { return recordOn; }    //KSG Edit.  Added so the mode can be changed from Winder.StartLayer()
                                  set { recordOn = value; } }
    private static bool xservoOn;         // false => off
    public static bool XservoOn { get { return xservoOn; }    //KSG Edit.  Added so the mode can be changed from Winder.StartLayer()
                                  set { xservoOn = value; } }
    private static bool yservoOn;         // false => off
    public static bool YservoOn { get { return yservoOn; }    //KSG Edit.  Added so the mode can be changed from Winder.StartLayer()
                                  set { yservoOn = value; } }
    ...
    private static void ButtonAction()
    {
      ...
      for (int nbb = 3; nbb < 21; nbb++)  // nbb is button-box button number -- KSG changed to 21 instead of 18 for more buttons
      {
