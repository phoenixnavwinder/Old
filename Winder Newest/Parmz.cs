...
namespace Zlib
{
  ...
  --------------------------------------------------------------------------*/
    public bool Save(string filePath, double[] newValues)
    {
        ...
        //if ammounts wrong return false
        ...
        //loop throught the lines in the file.  Any line that doesn't start with // should be parsed, and its value replaced with the new one
        ...
        for (int i = 0; i < newLines.Count(); i++)
        {
            ...
            else
            {
                ...
                substring = substring.Replace(tempHolder, newValues[newValueIndex].ToString());

                newLines[i] = newLines[i].Remove(valuePossition, newLines[i].Length - valuePossition);

                newLines[i] += substring;

                newValueIndex++;
            }
        }
            //write all text back to file
        try
        {
