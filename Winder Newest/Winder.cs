/*------------------------------------------------------------------------------
File: Winder.cs                                        Updated: 06/04/2013
...
namespace WinderProject
{
  class Winder
  {
    ...
    private static bool rescueMode;     //KSG ADDED
    ...
    private static bool isExperimentalMode;  //used to track if experimental mode is being used to modify coil paramz last x layers
    private static int exprimentalLayers;  //Used to track how many layers to modify coil paramz for experimenal mode
    ...
    private static void Pause()
    {
      ...
      st = "P -> position, W -> wind, E -> end layer, S -> status" 
            + ", V -> toggle VR, T -> terminate, R -> rescue, C -> edit coil.paramz, X -> toggle experimental mode";
      Console.WriteLine(st);
      st = Console.ReadLine();
      ...
      switch (ch)
      {
        ...
        case 'T':  // terminate
          operating = false;
          break;
        case 'R':  // KSG edit: go to rescue mode
          Util.W2("Transition to rescue mode");
          rescueMode = true;
          windMode = true;
          break;
        case 'C': // KSG edit: open CoilParameters form to edit coil.paramz
          frm_CoilParameters coilParamz = new frm_CoilParameters(lz);
 
          System.Windows.Forms.Application.Run(coilParamz);

          if (CoilParmzRead())
          {
            Console.WriteLine(" -- coil parameters files read");
          }
          else
          {
            Util.W2W("Coil parameters file read error -- terminating");
            Terminate();
          }

          pauseMode = true;
          break;
        case 'X':  // toggle experimenal mode
          isExperimentalMode = !isExperimentalMode;
          if (isExperimentalMode)
          {
              Console.WriteLine(" -- experimental mode is enabled.");
              Console.WriteLine(" -- How many layers at the end of the wind should use modified coil parmaters?");
              try
              {
                  exprimentalLayers = Int32.Parse(Console.ReadLine());
                  Console.WriteLine(" -- Experimental mode will use " + exprimentalLayers.ToString() +" layers.");
              }
              catch
              {
                  Console.WriteLine(" -- Invalid value.  Experimental mode will use 4 layers.");
              }
              
          }
          else
          {
              Console.WriteLine(" -- experimental mode is disabled.");
          }
          pauseMode = true;
          break;
        default:
    ...
    /*--------------------------------------------------------------------------
   Rescue:Added by KSG.  Called by Wind() is rescue mode was activated.  This 
     * asks for a layer, other than 0, to start on.  This allows for the user to
     * continue a wind that was interupted.
  --------------------------------------------------------------------------*/
    private static void Rescue()
    {
        bool errorz = true; //assume something will go wrong...

        while (errorz)
        {
            string st = "Rescue Mode has been selected.  What layer would you like to start on?";
            Console.WriteLine(st);
            string stLayer = Console.ReadLine();

            st = "Enter the number of turns that were in layer 1";
            Console.WriteLine(st);
            string stTurns = Console.ReadLine();

            try
            {
                int i = Int32.Parse(stLayer);
                layerNumber = i - 1;

                baseLayerTurnsAct = Int32.Parse(stTurns);

                errorz = false; //Valid layer entered and .  Continue
            }
            catch (Exception ex)  //Invalid layer.  Loop
            {
                st = "Invalid layer or base turns";
                lz.Sl("Error chosing layer or turns in rescue mode");
                lz.Bl();
                lz.Sl(ex.ToString());
            }
        }
    ...
    private static void StartLayer()
    {
      ...
      m = m - 2 * (m / 2);  // m is 0 if (layerNumber/2) is even,
        ...
        //This checks for experimental mode.  If experimental mode is enabled and the process is at the correct layer
        //The coil parameters will be modified
      if (isExperimentalMode && (layerNumber == coilLayers - exprimentalLayers))
      {
          //divide turns by 4, round up....
          fiberDiam = fiberDiam * 4;
          //adjust number of layers based on how many layers are left * 4.  Do this when wind begins.
      }
      ...
      while (!canCorrect) //Begin Loop
      {
          ...
          {
              setup = "Position machine so that the X can is left, Insert pin in X tensioner, Turn Y tensioner power on, Press enter to continue";
          ...
          {
              setup = "Rotate machine so that Y can is left, Remove pin from X tensioner, Turn X tensioner power on, Press enter to continue";
              Console.WriteLine(setup);
              setup = Console.ReadLine();
          ...
          else // any other layer
          {
              ...
              switch (r)
              {
                  case 1:  // layer 1, 5, 9, 13, 17 etc
                      setup = "Position machine so the X can is left.  Press enter to continue";
                      Console.WriteLine(setup);
                      setup = Console.ReadLine();
                      CheckCorrectCanPossition(1);
                      break;
                  case 2:  // layer 2, 6, 10, 14, 18 etc
                      setup = "Rotate machine so the Y can is left.  Press enter to continue";
                      Console.WriteLine(setup);
                      setup = Console.ReadLine();
                      CheckCorrectCanPossition(2);
                      break;
                  case 3:  // layer 3, 7, 11, 15, 19
                      setup = "Position machine so the Y can is left.  Press enter to continue";
                      Console.WriteLine(setup);
                      setup = Console.ReadLine();
                      CheckCorrectCanPossition(2);
                      break;
                  case 0:  // layer 4, 8, 12, 16, 20 etc
                      setup = "Rotate machine so the X can is left.  Press enter to continue";
                      Console.WriteLine(setup);
                      setup = Console.ReadLine();
                      CheckCorrectCanPossition(1);
                      break;
  ...
  --------------------------------------------------------------------------*/
    ...
    private static void EndLayer()
    {
      ...
      if (layerNumber == 1)
      {
        ...
        //Added by KSG.  This puts the active can in park position at the end of the layer if the lazysusan will be rotated.  Otherwise puts the active can in left start possition.  Then whenever a new layer is started we are in a predictable possition.
      ...
      {
          case 1:  // layer 1, 5, 9, 13, 17 etc             
              Mbox.RecordGoTo(false, true, 3);  // ({true for record, false for goto}, {true if x left, false if y left}, {1 for right start, 2 for left start, 3 for park, 4 for home theta})
              break;
          case 2:  // layer 2, 6, 10, 14, 18 etc
              Mbox.RecordGoTo(false, false, 2);  // ({true for record, false for goto}, {true if x left, false if y left}, {1 for right start, 2 for left start, 3 for park, 4 for home theta})
              break;
          case 3:  // layer 3, 7, 11, 15, 19
              Mbox.RecordGoTo(false, false, 3);  // ({true for record, false for goto}, {true if x left, false if y left}, {1 for right start, 2 for left start, 3 for park, 4 for home theta})
              break;
          case 0:  // layer 4, 8, 12, 16, 20 etc
              Mbox.RecordGoTo(false, true, 2);  // ({true for record, false for goto}, {true if x left, false if y left}, {1 for right start, 2 for left start, 3 for park, 4 for home theta})
              break;
    ...
    private static void Setup()
    {
      ...
      //KSG ADDED.  Set rescueMode to false
      rescueMode = false;
