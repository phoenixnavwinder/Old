/*------------------------------------------------------------------------------
File: Bbox.cs                                          Updated: Apr. 14, 2013
...
namespace WinderProject
{
  class Bbox
  {
    ...
    */
    ...
    public  static bool ActiveBreakOn   { get { return activeBreakOn; }    //zz
                                          set { activeBreakOn = value; } } //zz

    private static bool inactiveBreakOn;  // false => off
    public  static bool InactiveBreakOn { get { return inactiveBreakOn; }  //zz
                                          set { InactiveBreakOn = value; }}//zz

    private static bool recordOn;         // false => goto is on
    public static  bool RecordOn        { get { return recordOn; }         //zz
                                          set { RecordOn = value; }  }     //zz

    private static bool xservoOn;         // false => off
    public static  bool XservoOn        { get {return xservoOn;}           //zz
                                          set {XservoOn = value; }}        //zz

    private static bool yservoOn;         // false => off
    public static  bool YservoOn        { get { return xservoOn; }         //zz
                                          set { XservoOn = value; }}       //zz
    ...
    public static void SampleSetup()
    {
      // Set BufferSize in order to use buffered data.
        joystick.Properties.BufferSize = BuffSize;   //todo no hardware todo // 

      // Acquire the joystick.
        joystick.Acquire();  //todo no hardware todo // 
    }  // end SampleSetup
    ...
    private static void ButtonAction()
    {
      bool xa;
     // bool xb;
      for (int nbb = 3; nbb < 21; nbb++)  // nbb is button-box button number  to 21 not 18
      {
        ...
        {    
          ...
          switch (nbb)  // switch on the button box button number
          {
            ...
            case 6:  // toggle joystick fast/slow
              ...
              break;  // end of thing
              case 21: ; // message that it works
                  Console.WriteLine( "button 21 pushed");
                  break;
              case 20:  ; //Message that iterator works
                   Console.WriteLine(" button 20 pushed");
                    recordOn = false; 
              Ledz.SetLEDState(2, recordOn);
              Console.WriteLine(" -- goto set on"  ); //say we are in go to mode
                   bool lazyClosed = Winder.LazySusan();// make sure xactive set
 			  xa = Winder.Xactive;    // make the right side selection
              activeBreakOn = true;
              inactiveBreakOn = false;
              ...
              //xa = xa; //
             Ledz.SetLEDState(4, inactiveBreakOn);  // 3 to 4
              Mbox.SetBrake(false, inactiveBreakOn);  //OFF???
		      Thread.Sleep(200); // WAIT 0.2 SECOND
              // record/goto park Z for active side
              Mbox.RecordGoTo(recordOn, xa, 3);
              Console.WriteLine("should have parked guide"); //

            Console.WriteLine(" -- REMOVE LOCK PIN, TURN ON X-TENSIONER, ROTATE MACHINE.");
              Console.WriteLine(" --  WHEN DONE WITH ROTATION HIT ENTER or else");
              Console.ReadLine(); //
              xa = !xa; //

      //Note that since we havent left Bbox we havent sampled L vs R state switches and Winder.Xactive is unchanged.  We need to put this in.  This would be a good place to insert commands which increment the layer  rather than having to leave wind and go to end.
              activeBreakOn = false;
              inactiveBreakOn = true;
                   Ledz.SetLEDState(3, activeBreakOn);
                   Mbox.SetBrake(true, activeBreakOn);
                  Thread.Sleep(300); // WAIt .3 SECOND 
                  Ledz.SetLEDState(3, inactiveBreakOn);
                   Mbox.SetBrake(true, inactiveBreakOn);
                  Thread.Sleep(300); // WAIt .3 SECOND
                  // Console.ReadLine(); //  Wait for the rotation
                //  SetVirtualRotation(true);
                //   activeBreakOn = true;
                  //    inactiveBreakOn = false;
                   Ledz.SetLEDState(3, activeBreakOn);
              Mbox.SetBrake(true, activeBreakOn); 
                   Thread.Sleep(300); // WAIT 1/3 SECOND
              Ledz.SetLEDState(3, inactiveBreakOn);
             ...
             // inactiveBreakOn = false;
                   Ledz.SetLEDState(3, activeBreakOn);
              Mbox.SetBrake(true, activeBreakOn);
              Console.WriteLine(" --  done with 20");
             // SetVirtualRotation(false);
              xa = !xa; //
if (xa)  // Y is the inactive side
              ...
              else  // X is the inactive side
              {
                  ...
                  Thread.Sleep(250); // WAIT more
              }
              // toggle active side servo on
            // xa = Winder.Xactive;
              if (xa)  // Y is the inactive side
              {
                yservoOn = true;
		      ...
		      Console.WriteLine(" -- READY TO START EVEN LAYER") ;
  // record/goto home theta for active side
              ...
              // toggle inactive side servo on/off
  ...
  // record/goto right start position for active side
              
              Mbox.RecordGoTo(recordOn, xa, 1); //
                            break;  //

              case 7: ;   // Odd to even
              // SetRecordOn(false);   // must be in go to mode. // zz removed Al line
              recordOn = false; 
              Ledz.SetLEDState(2, recordOn);
              Console.WriteLine(" -- goto set on set off"  ); //say we are in go to mode

              
 			  xa = Winder.Xactive;    // make the right side selection
              Mbox.RecordGoTo(recordOn, xa, 4);   // home theta  (14)  
              Mbox.RecordGoTo(recordOn, xa, 3);   // park Z (13)

              Console.WriteLine(" -- REMOVE LOCK PIN, TURN ON X-TENSIONER, ROTATE MACHINE.");
              Console.WriteLine(" --  WHEN DONE WITH ROTATION HIT ENTER");
		      Console.ReadLine() ; //WE WAIT HERE UNTIL THE ROTATION IS DONE.
                                   //this command will be replaced with a sense the switch command.

              lazyClosed = Winder.LazySusan(); // make sure xactive set
 			    // (Al) swap ends  no  !xa
              ...
              Mbox.RecordGoTo(recordOn, xa, 1); // go to right flange start
		
             // turn inactive side servo to park
            //  xa = Winder.Xactive;
              if (xa)  // Y is the inactive side
              {
                  yservoOn = false;
  ...
  // record/goto right start position for active side
            ...
            case 15:  // toggle inactive side servo on/off
              ...
              break;
            case 16:  // EVEN TO ODD SWITCH nO RTOTATION Of MACHINE
                      // OPERATOR HIT PAUSE, e FOR END OF LAYER AND
                      // w TO GO BACK TO 
                      // WIND  THESE ACTIVITIES NEED TO BE PART OF THIS BUTTON PUSH COMMAND
                      // goto left start position for active side
              //lazyClosed = Winder.LazySusan(); // make sure xactive set
     		  xa = Winder.Xactive;    // (Al) make the right side selection

              Mbox.RecordGoTo(recordOn, xa, 2);//  LEFT STArt
		   //
   Util.Delay(250);  // zz remove Al 	Thread.Sleep(250) // WAIT more
 			  Mbox.RecordGoTo(recordOn, xa, 4);  // go to tdc
              //lz.Sl(" -- record on is " + recordOn.ToString() 
              //    + " -- xs        is " + recordOn.ToString());
